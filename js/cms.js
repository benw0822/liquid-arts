...
const slugInput = document.getElementById('article-slug'); // [NEW] Link to slug input
...
function updateTOC() {
    const headers = [];
    // Quill stores headers as attributes, but we scan DOM for simplicity
...
saveBtn.addEventListener('click', async () => {
    ...
    // Basic regex: only lowercase letters, numbers, and hyphens (allow empty)
    if (slug && !/^[a-z0-9-]+$/.test(slug)) {
        alert('Invalid Slug: Only lowercase letters, numbers, and hyphens are allowed.');
        return;
    }
    if (slug === '') slug = null;
    ...
    try {
        ...
        if (err.code === '23505' && err.message.includes('slug')) {
            alert('Error: This URL slug is already taken. Please choose another one.');
        } else {

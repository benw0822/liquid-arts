document.addEventListener('DOMContentLoaded', () => {
    ...
    async function loadDashboardStats() {
        ...
        if (latestBars) {
            ...
            // Run in background, do not await the whole promise chain here
            (async () => {
                for (const b of latestBars) {
                    // Only fetch if we need to refine it (e.g. if we suspect the location string is inadequate or we prefer standardized city)
                    // Or, just fetch to be sure, but do it silently.
                    if (b.lat && b.lng) {
